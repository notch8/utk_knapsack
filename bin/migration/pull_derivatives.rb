...
MANIFEST = ENV.fetch('DERIVATIVE_MANIFEST', File.expand_path('../../tmp/migration/manifests/derivatives_pulled.txt', __dir__))
...
def local_root
  out, status = Open3.capture2e('docker', 'compose', 'exec', '-T', 'web', 'bash', '-lc',
                                'cd /app/samvera && ./bin/rails runner "print Hyrax.config.derivatives_path"')
  abort "could not read the local derivatives path: #{out[0, 300]}" unless status.success?
