...
references them by identifier (`parents: collections:ruskin`).
...
5. **Ingest** with Bulkrax and the migration factory.  Members are ordered by `sequence` at the end
   of the run, automatically; see **Member order**.
6. **Reconcile**.
...
`{name, uri, role}`.  `sheet_transform.rb` builds its role map from the two role authorities: a
